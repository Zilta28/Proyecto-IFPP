...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE aceptacion_ss SET directivo = ?, cargo = ?, escuela = ?, prestatario = ?, carrera = ?, matricula = ?, horas = ? WHERE Id_ss = ?";
    ...
    if ($afectado == 1) {
        echo "<script>alert('Registro actualizado correctamente'); location.href='ss_table.php';</script>";
        exit();
    } else {
        echo "<script>alert('Error al actualizar el registro'); location.href='ss_table.php';</script>";
        exit();
    }
}
...
if (!$fila) {
    echo "<script>alert('Registro no encontrado'); location.href='ss_table.php';</script>";
    exit();
}
?>
