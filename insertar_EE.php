...
if (isset($_POST["registrar"])) {
    ...
    if (move_uploaded_file($fotoTmpName, $fotoRuta)) {
        $query = "INSERT INTO aceptacion_ee (directivo, cargo, escuela, prestatario, carrera, matricula, horas, ruta, telefono, dj) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($afectado == 1) {
            ...
            alert(message);
            location.href = 'Form_EE.html';
        </script>";
        

        } else {
            echo "<script> alert('El prestatario [$prestatario] NO se agregó correctamente'); location.href='Form_EE.html';</script>";
        }
