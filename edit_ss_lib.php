...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE terminacion_ss  SET nombreExpide = ?, cargoPer = ?, nombreEscuela = ?, nombreEstudiante = ?, carrera = ?, noCuenta = ?, area = ?, subarea = ?,jefeInmediato = ?, numHoras = ?, periodo_inicio = ?, periodo_final = ?  WHERE Id_ss = ?";
    ...
    if ($afectado == 1) {
        echo "<script>alert('Registro actualizado correctamente'); location.href='ss_lib_table.php';</script>";
        exit();
    } else {
        echo "<script>alert('Error al actualizar el registro'); location.href='ss_lib_table.php';</script>";
        exit();
    }
}
...
if (!$fila) {
    echo "<script>alert('Registro no encontrado'); location.href='ss_lib_table.php';</script>";
    exit();
}
?>
  ...
  <h1>Edicion de registro</h1>
    ...
    <form method="POST" action="">
        <div class="form-group">
            <input type="hidden" class="form-control" name="id" value="<?php echo $fila['Id_ss']; ?>">
          </div>
      <div class="form-group">
