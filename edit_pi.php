...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE aceptacion_pi SET directivo = ?, cargo = ?, escuela = ?, prestatario = ?, carrera = ?, matricula = ?, horas = ? WHERE Id_pi = ?";
    ...
    if ($afectado == 1) {
        echo "<script>alert('Registro actualizado correctamente'); location.href='pi_table.php';</script>";
        exit();
    } else {
        echo "<script>alert('Error al actualizar el registro'); location.href='pi_table.php';</script>";
        exit();
    }
}
...
if (!$fila) {
    echo "<script>alert('Registro no encontrado'); location.href='pi_table.php';</script>";
    exit();
}
?>
