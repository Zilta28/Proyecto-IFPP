...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE terminacion_ep  SET nombreExpide = ?, cargoPer = ?, nombreEscuela = ?, nombreEstudiante = ?, carrera = ?, noCuenta = ?, area = ?, subarea = ?, jefeInmediato = ?, numHoras = ?, periodo_inicio = ?, periodo_final = ?  WHERE Id_ep = ?";
    ...
    if ($afectado == 1) {
        echo "<script>alert('Registro actualizado correctamente'); location.href='ep_lib_table.php';</script>";
        exit();
    } else {
        echo "<script>alert('Error al actualizar el registro'); location.href='ep_lib_table.php';</script>";
        exit();
    }
}
...
if (!$fila) {
    echo "<script>alert('Registro no encontrado'); location.href='ep_lib_table.php';</script>";
    exit();
}
?>
  ...
  <h1>Edicion de registro</h1>
    ...
    <form method="POST" action="">
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control" name="jefeInmediato" value="<?php echo $fila['jefeInmediato']; ?>">
      </div>
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control" name="periodo inicio" value="<?php echo $fila['periodo_inicio']; ?>">
      </div>
      
      <div class="form-group">
        <label for="periodo_final">Periodo final: </label>
        <input type="text" class="form-control" name="periodo final" value="<?php echo $fila['periodo_final']; ?>">
      </div>
