...
if(isset($_POST["registrar"])){
    ...
    $query = "INSERT INTO terminacion_ep (nombreExpide, cargoPer, nombreEscuela, nombreEstudiante, carrera, noCuenta, area, subarea, jefeInmediato , numHoras, periodo_inicio, periodo_final, ruta) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if($afectado == 1){
        ...
        alert(message);
        location.href = 'Form_EP_Lib.html';
    </script>";
    }
    else{
        echo "<script> alert('El prestatario [$nombreEstudiante] NO se agregó correctamente'); location.href='Form_EP_Lib.html';</script>";
    }
