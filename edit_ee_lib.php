...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE terminacion_ee  SET nombreExpide = ?, cargoPer = ?, nombreEscuela = ?, nombreEstudiante = ?, carrera = ?, noCuenta = ?, area = ?, subarea = ?,jefeInmediato = ?, numHoras = ?, periodo_inicio = ?, periodo_final = ?  WHERE Id_ee = ?";
    ...
    if ($afectado == 1) {
        echo "<script>alert('Registro actualizado correctamente'); location.href='ee_lib_table.php';</script>";
        exit();
    } else {
        echo "<script>alert('Error al actualizar el registro'); location.href='ee_lib_table.php';</script>";
        exit();
    }
}
...
if (!$fila) {
    echo "<script>alert('Registro no encontrado'); location.href='ee_lib_table.php';</script>";
    exit();
}
?>
  ...
  <h1>Edicion de registro</h1>
    ...
    <form method="POST" action="">
        <div class="form-group">
            <input type="hidden" class="form-control" name="id" value="<?php echo $fila['Id_ee']; ?>">
          </div>
      <div class="form-group">
