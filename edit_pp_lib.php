...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE terminacion_pp  SET nombreExpide = ?, cargoPer = ?, nombreEscuela = ?, nombreEstudiante = ?, carrera = ?, noCuenta = ?, area = ?, subarea = ? , jefeInmediato = ?, numHoras = ?, periodo_inicio = ?, periodo_final = ?  WHERE Id_pp = ?";
    ...
    if ($afectado == 1) {
        echo "<script>alert('Registro actualizado correctamente'); location.href='pp_lib_table.php';</script>";
        exit();
    } else {
        echo "<script>alert('Error al actualizar el registro'); location.href='pp_lib_table.php';</script>";
        exit();
    }
}
...
if (!$fila) {
    echo "<script>alert('Registro no encontrado'); location.href='pp_lib_table.php';</script>";
    exit();
}
?>
