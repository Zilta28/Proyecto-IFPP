...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE aceptacion_ee SET directivo = ?, cargo = ?, escuela = ?, prestatario = ?, carrera = ?, matricula = ?, horas = ? WHERE Id_ee = ?";
    ...
    if ($afectado == 1) {
        echo "<script>alert('Registro actualizado correctamente'); location.href='ee_table.php';</script>";
        exit();
    } else {
        echo "<script>alert('Error al actualizar el registro'); location.href='ee_table.php';</script>";
        exit();
    }
}
...
if (!$fila) {
    echo "<script>alert('Registro no encontrado'); location.href='ee_table.php';</script>";
    exit();
}
?>
